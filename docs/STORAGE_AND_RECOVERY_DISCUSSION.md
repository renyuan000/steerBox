...
四个专业方向都应能以多年生命周期运行，但“长期运行”属于共享 Harness 的运行时契约，不属于某个领域 agent 的私有实现。运行时至少应支持四种任务形态：

- 交互任务：用户随时聊天、查询、下达任务并接入已有上下文；
- 周期任务：按分钟、小时、天、周或自定义周期巡检、分析、验证和复盘；
- 事件驱动任务：由代码变化、服务异常、漏洞情报、攻击告警或资产变化触发；
- 常驻运营任务：后台持续监听数据源，维护风险、调查、处置和验证状态。

长期运行必须以持久化事实为基础，而不是依赖某个模型上下文或进程存活。每个长期任务都应能关联 `Task / TaskRun / Attempt`、心跳、租约、预算、授权期限、检查点、证据和副作用记录；服务重启、网络中断、模型切换或版本升级后，只能从已记录的合法边界恢复。

安全评估与漏洞研究任务的授权必须有有效期和明确范围，不能因为后台常驻而变成永久权限。安全运营任务的高风险响应必须经过 `PolicyGate`、审批或人工接管；低风险自动化也必须可暂停、可审计并能验证效果。

架构目标是 7x24 常驻和多年可恢复；第一阶段仅验证本地单机、顺序执行、事件落盘和最小恢复检查点。分布式调度、高可用故障转移、跨节点租约和完整常驻运营服务属于后续阶段。
