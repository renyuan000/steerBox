...
- 你现在定的 `一个项目 + 软件开发智能体 / 安全智能体 两条主线`，和三篇文章的精神是一致的。
- 共享的应该是 harness 核心：
  - 长时执行
  - steering
  - 工具控制
  ...
  - 验证闭环
- 分化的应该是领域能力：
  - 开发方向：代码、测试、构建、review、refactor
  - 安全方向：日志、事件、分析、响应、处置、审批
...
- 如果这几类对象没有先站稳，后面无论做开发智能体还是安全智能体，都会很快重新跌回“靠 prompt 硬顶”的状态。
